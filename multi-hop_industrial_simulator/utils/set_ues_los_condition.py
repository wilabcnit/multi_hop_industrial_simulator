...
# Function to determine and set the LoS/NLoS condition for each UE w.r.t. the BS and other UEs
def set_ues_los_condition(ue_array: ndarray, bs: BS, machine_array: ndarray, machine_kind: str, link : str):
    """
    ...
    for ue in ue_array:
        ...
        if link == 'ue_ue' and ue_distribution_type == 'Custom' and machine_kind == 'empty' and ue.id_machine != 'False' and ue.id_machine == id_machine:
...
# Function to determine and set the LoS/NLoS condition for a specific UE w.r.t. the BS
def unique_set_ues_los_condition_ue_bs(x: float, y: float, z: float, bs: BS, machine_array: ndarray):
    """
    Determine if there is a Line-of-Sight (LoS) or Non-Line-of-Sight (NLoS) condition between
    the BS (Base Station) and UEs considering obstructions (machines).
    ...
    # Pick-up UE coordinates
    x_ue = x  # ue_coordinates[0]
    y_ue = y  # ue_coordinates[1]
    z_ue = z  # ue_coordinates[2]
    x_diff = abs(x_bs - x_ue)
    y_diff = abs(y_bs - y_ue)

    step = 0.001  # machines[0].get_machine_size() / 2
...
def set_ues_los_condition_new(ue: Ue, bs: BS, machine_array: ndarray, link : str):
    """
    Determine if there is a Line-of-Sight (LoS) or Non-Line-of-Sight (NLoS) condition between UEs
    considering obstructions (machines).
