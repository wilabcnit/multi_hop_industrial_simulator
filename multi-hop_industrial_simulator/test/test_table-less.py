...
from timessim.utils.check_success import check_collision
from timessim.utils.check_success import check_collision_bs

from timessim.utils.check_phy_success import check_phy_success
from timessim.utils.compute_distance_m import compute_distance_m
from timessim.utils.compute_propagation_delays import compute_propagation_delays
from timessim.utils.compute_simulation_outputs import compute_simulator_outputs
from timessim.utils.compute_snr_threshold import compute_snr_threshold_db
from timessim.utils.instantiate_bs import instantiate_bs
from timessim.utils.instantiate_ris import instantiate_ris
from timessim.utils.read_input_file import read_input_file
from timessim.utils.read_inputs import read_inputs
from timessim.env.geometry import Geometry
from timessim.env.distribution import Distribution
from timessim.utils.instantiate_ues import instantiate_ues
from timessim.utils.compute_simulator_tick_duration import compute_simulator_tick_duration
from timessim.channel_models.THz_channel import THzChannel
from timessim.utils.check_for_neighbours import check_for_neighbours

from timessim.utils.set_ues_los_condition_vecchio import set_ues_los_condition_ue_bs, \
    set_ues_los_condition_ue_ue, set_ues_los_condition_ue_bs_unif, set_ues_los_condition_ue_ue_unif
from timessim.utils.set_ues_los_condition import set_ues_los_condition_new

from timessim.utils.plot_data import plot_factory, plot_snr, plot_scenario_2d, write_data

from collections import deque

from timessim.utils.choose_next_action import choose_next_action_tb_no_RL
from timessim.utils.compute_max_prop_delay_tx_range import compute_max_prop_delay_tx_range
...
mobility = inputs.get('simulation').get('mobility')
ack_tx_bs_seen = 0
...
output_keys = ["p_mac", "s_ue", "s", "l", "e", "j_index", "Discarded_packets_percentage"]
...
for seed in range(initial_seed, final_seed + 1):
    ...
    for n_ue_index, n_ue in enumerate(range(initial_number_of_ues, final_number_of_ues + 1, step_number_of_ues)):
        ...
        for n_simulation in range(n_simulations):
            ...
            #              save_file=f'./timessim/results/plot_{scenario_name}_scenario_3d.png')
            ...
            #                  distribution_class=distribution_class,
            #                  scenario_name=scenario_name,
            #                  save_file=f'./timessim/results/plot_{scenario_name}_scenario_2d.png',
            #                  )
...
final_file_name_output_1 = 'timessim/results/' + f'{year}_{month:02d}_{day:02d}_' + 'TL_output_dict' + '_final_UEs_' + str(final_number_of_ues) + '_Payload_' + str(payload_fq)+ '_contention_window_' + str(contention_window_int) + '_buffer_size_' + str(max_n_packets_to_be_forwarded) + '.txt'
final_file_name_output_2 = 'timessim/results/' + f'{year}_{month:02d}_{day:02d}_' + 'TL_averaged_data' + '_final_UEs_' + str(final_number_of_ues) +'_Payload_' + str(payload_fq)+ '_contention_window_' + str(contention_window_int) + '_buffer_size_' + str(max_n_packets_to_be_forwarded) + '.txt'
final_file_name_output_3 = 'timessim/results/' + f'{year}_{month:02d}_{day:02d}_' + 'TL_inputs' + '_final_UEs_' + str(final_number_of_ues) + '_Payload_' + str(payload_fq)+ '_contention_window_' + str(contention_window_int) + '_buffer_size_' + str(max_n_packets_to_be_forwarded) + '.txt'
