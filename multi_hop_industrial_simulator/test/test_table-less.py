...
from multi_hop_industrial_simulator.utils.compute_max_prop_delay_tx_range import compute_max_prop_delay_tx_range
...
# Get the duration of the BO state
...
def create_simulator_timing_structure(input_n_ue: int, input_simulation_duration_tick: int):
    ...
    Args:
      input_n_ue: int: number of UEs in the network
      input_simulation_duration_tick: int: duration of the simulation in ticks
        ...
        for j in range(0, input_n_ue):
            if j != i:
                ue_value['DATA_RX']['UE_' + str(j)] = np.array([[input_simulation_duration_tick + 1] * 4],
                                                               dtype=int)  # starting tick, ending tick, DATA size,
                # packet ID
...
def reset_simulator_timing_structure(output_simulator_timing_structure: dict, input_simulation_duration_tick: int):
    ...
    Args:
      output_simulator_timing_structure: dict: simulator timing structure
      input_simulation_duration_tick: int: duration of simulation in ticks
...
def insert_item_in_timing_structure(input_simulator_timing_structure: dict, input_starting_tick: int,
    ...
    Args:
      input_simulator_timing_structure: dict: simulator timing structure
      input_starting_tick: int: starting tick of packet RX
      input_final_tick: int: final tick of packet RX
      input_third_field: int: third field of the simulator timing structure
      input_fourth_field: object: fourth field of the simulator timing structure
      input_tx_key: str: transmitter
      input_type_key: str: key of the dictionary
      input_rx_key: str: receiver
...
def remove_item_in_timing_structure(input_simulator_timing_structure: dict, input_tx_key: str, input_type_key: str,
    ...
    Args:
      ...
      input_type_key: str: key of the dictionary
      input_rx_key: str: receiver
      index: int:  index to be removed from the simulator structure (Default = None)
...
def find_data_rx_times_tick(input_simulator_timing_structure: dict, input_ue_id: int, current_tick: int):
    ...
    Args:
      input_simulator_timing_structure: dict: simulator timing structure
      input_ue_id: int: UE_ID
      current_tick: int: current tick of simulation

    Returns:
        starting tick of DATA RX, final ticks of DATA RX, DATA size in bytes, packet ID, UE_ID
...
def find_data_rx_times_at_bs_tick(input_simulator_timing_structure: dict, current_tick: int):
    ...
    Returns:
        starting and final tick of DATA RX at BS, packet ID, UE_ID sending DATA
...
def find_ack_rx_times_at_bs_tick(input_simulator_timing_structure: dict, current_tick: int):
    ...
    Args:
      input_simulator_timing_structure: dict: input simulator timing structure
      current_tick: int: current simulation tick

    Returns:
        Starting and final ACK reception ticks at BS, UE_ID dest of ACK, ID of the DATA for which ACK is sent,
        ACK transmitter, number of ACKs received simultaneously
...
# Function to find a ACK RX inside the simulator timing structure at a given tick for the UE
def find_ack_rx_times_tick(input_simulator_timing_structure: dict, input_ue_id: int, current_tick: int):
    ...
    Args:
      input_simulator_timing_structure: dict: input simulator timing structure
      input_ue_id: int: UE_ID
      current_tick: int: current simulation ticks

    Returns:
        Starting and Final tick of ACK RX, list of ACK sources, UE_ID final dest of ACK, packet ID for which the ACK is
        sent
...
if scenario_name in ['birex', 'aetna', 'custom', 'aetna_wcnc', 'aetna_eucnc', 'aetna_deliverable', 'grid']:
...
for seed in range(initial_seed, final_seed + 1):
    ...
    for n_ue_index, n_ue in enumerate(range(initial_number_of_ues, final_number_of_ues + 1, step_number_of_ues)):
        ...
        # this is the function to use as a prop delay the max transmission range

        max_prop_delay_tx_range = compute_max_prop_delay_tx_range(ue=ue_array[0],
                                                                  bs=bs,
        ...
        for n_simulation in range(n_simulations):
            ...
            for ue_index, ue in enumerate(ue_array):
                ...
                # ue.obs[3]: Time To Live of each neighbor
                ue.reset_obs()
                ue.reset_temp_obs()
                ue.set_last_action(input_last_action=None)
                ue.set_broadcast_bool(input_broadcast_bool=False)
            ...
            """
            # Code to plot the factory in 2D and 3D
            ...
            while t <= tot_simulation_time_tick:
                # Mobility code: there are pre-defined instant of time where the UEs within the environment move ->
                # we randomly shuffle the UEs coordinates and it's important to update the simulator timing structure
                # to save the old state of each UE and update the current state based on the new coordinates
                ...
                if t == next_t_change and t > 0 and mobility_shuffle is True:
                    t_change = t
                    for ue in ue_array:
                                ...
                                for ue_key_int in simulator_timing_structure[ue_key_ext]['DATA_RX'].keys():
                                    ...
                                    if len(simulator_timing_structure[ue_key_ext]['DATA_RX'][ue_key_int]) > 0:
                                        for array_index in range(
                                            ...
                                            if simulator_timing_structure[ue_key_ext]['DATA_RX'][ue_key_int][array_index][0] \
                                                ...
                                                for other_ue in ue_array:
                                                    if ue_key_int == f'UE_{other_ue.get_ue_id()}':
                                                        ...
                                                        prop_delay_tick = ue.get_prop_delay_to_ue_tick(other_ue.get_ue_id())
                                ...
                                for ue_key_int in simulator_timing_structure[ue_key_ext]['ACK_RX'].keys():
                                    ...
                                    if len(simulator_timing_structure[ue_key_ext]['ACK_RX'][ue_key_int]) > 0:
                                        for array_index in range(
                                            ...
                                            if simulator_timing_structure[ue_key_ext]['ACK_RX'][ue_key_int][array_index][0] \
                                                    != tot_simulation_time_tick + 1 and \
                    ...
                    # Code to plot again the factory and see the new coordinates of the UEs
                    ...
                    # Update the LOS / NLOS conditions of the UEs
                    for i in range(0, len(ue_array)):
                    ...
                    for j in range(0, len(ue_array)):
                        for i in range(0, len(ue_array)):
                            los_condition = set_ues_los_condition(ue=ue_array[j], bs=ue_array[i],
                                                                  ...
                                                                  link='ue_ue')
                            ue_array[j].is_in_los_ues.append(los_condition)
                    # Update the propagation delays based on the new coordinates
                    compute_propagation_delays(ue_array=ue_array, bs=bs,
                                               input_simulator_tick_duration_s=simulator_tick_duration_s)
                    # After the movement, update the simulator timing structure
                    for ue in ue_array:
                        for ue_key_ext in simulator_timing_structure.keys():
                            if ue_key_ext == f'UE_{ue.get_ue_id()}':
                                # Loop over the DATA receptions from other UEs
                                                    ...
                                                    if simulator_timing_structure[ue_key_ext]['DATA_RX'][ue_key_int][
                                                        ...
                                                        if enable_print:
                                                            print(" NEW start_tick = ",
                                                                  simulator_timing_structure[ue_key_ext]
                                                                  ['DATA_RX'][ue_key_int][array_index][0])
                                                            print("NEW end tick = ", simulator_timing_structure[ue_key_ext]
                                                            ['DATA_RX'][ue_key_int][array_index][1])
                        ...
                        if ue.get_state() == 'IDLE':
                            ...
                            if star_topology is False:
                                ...
                                if (ue.get_state_duration() == ue.get_state_final_tick() and
                                    ...
                                    for index in range(len(data_rx_at_ue_ue_id)): # iteration over the list of DATA received
                                        ...
                                        if len(ue.ues_interfering_at_ue) > 0:
                                            # for the interferers (whose that before where useful user),
                                ...
                                if star_topology is False:
                                    ...
                                    for packet in ue.ul_buffer.buffer_packet_list:
                                        if packet.get_data_to_be_forwarded_bool() is False:
                                            # check -> there is a data that has to be sent in broadcast
                                    ...
                                    if packet_id_to_be_sent != -1:  # I think this check is useless for full-queue.
                                        ...
                                        # fourth field is the ID of the packet received from a UE
                                        for packet in ue.ul_buffer.buffer_packet_list:
                                            t_data_ns = round((packet.packet_size * 8 * 1e-9) / bs.get_bit_rate_gbits(),
                                                11)  # 1.6 ns
                            ...
                            if ue.previous_state == 'WAIT_ACK':
                                ...
                                # Update the timing structure for UEs
                                for packet in ue.ul_buffer.buffer_packet_list:
                                    t_data_ns = round((packet.packet_size * 8 * 1e-9) / bs.get_bit_rate_gbits(), 11)  # 1.6 ns
                                    t_data_tick = round(t_data_ns / simulator_tick_duration_s)
                                    for other_ue in ue_array:
                                        if other_ue != ue:
                                            insert_item_in_timing_structure(
                            ...
                            if (ue.get_state_duration() == ue.get_state_final_tick() and
                                ...
                                if ue.ack_rx_during_wait_ack is True:
                                    ...
                                    ue.set_reception_during_wait_bool(input_data_rx_bool=False)
                                    (ack_rx_at_ue_starting_tick, ack_rx_at_ue_ending_tick,ack_rx_sources, ack_rx_dest, ack_rx_id) = (find_ack_rx_times_tick(
                                            input_simulator_timing_structure=simulator_timing_structure,
                                            current_tick=t,
                                            input_ue_id=ue.get_ue_id()))
                                        ...
                                        if success and ack_rx_dest[index] == ue.get_ue_id():
                                            ...
                                            if ack_received is False:
                                                ...
                                                if tx == bs:
                                                    ...
                                                    for packet in packet_to_be_removed:
                                                        if packet.packet_id == ack_rx_id[index]:
                                                            if packet.get_id() not in ue.list_data_generated_during_wait_ack and \
                                    ...
                                    if ue.ack_rx_with_success is True:
                                        remain_in_wait_ack = False


                                    for index in range(len(ack_rx_sources)):  # iteration over the list of ACKs received
                                        if len(simulator_timing_structure[f'UE_{ue.get_ue_id()}']['ACK_RX'][ack_rx_sources[index]]) > 1:

                                            # for index in range(len(ack_rx_id)):
                                            remove_item_in_timing_structure(
                                                input_simulator_timing_structure=simulator_timing_structure,
                                                input_rx_key=f'UE_{ue.get_ue_id()}',
                                                input_type_key='ACK_RX',
                                ...
                                if ue.data_rx_during_wait_ack is True and star_topology is False:
                                    ...
                                    for index in range(len(data_rx_at_ue_ue_id)): # iteration over the list of DATA received
                                        ...
                                        # Before adding a new packet ->
                                        # check if n_packets_buffer < max_n_packets_to_be_forwarded + 1
                ...
                if t == bs.get_state_duration():
                    ...
                    if bs.get_state() == 'RX':
                        ...
                        # Find the corresponding transmission
                        (data_rx_at_bs_starting_tick, data_rx_at_bs_ending_tick, output_data_rx_packet_id,
                         data_rx_at_bs_ue_id) = (find_data_rx_times_at_bs_tick(
                                input_simulator_timing_structure=simulator_timing_structure,
                                current_tick=t))
                        ...
                        if data_rx_at_bs_starting_tick is not None and data_rx_at_bs_ending_tick is not None and \
                            ...
                            for index in range(len(data_rx_at_bs_ue_id)):
                                ...
                                if success:
                                    ...
                                    for ue in ue_array:
                                        if ue.get_ue_id() == data_rx_at_bs_ue_id[index]:
                                            ...
                                            if len(ue.buffer_packet_sent) > 0:
                                                ...
                                                # Save in a list the ID of the UE for which the BS has received a
                                                # DATA (before going in ACK TX)
                                                ...
                                                for n_packet in range(len(ue.buffer_packet_sent)):
                                                    ...
                                                    if bs.sequence_number_of_packet_rx == ue.buffer_packet_sent[n_packet].get_id():
                                                        if ue.buffer_packet_sent[n_packet].get_id() not in \
                                                            ...
                                                            ack_packet_id = ue.buffer_packet_sent[n_packet].get_id()
                                                            ue.list_ack_sent_from_bs.append(
                                                                ue.buffer_packet_sent[n_packet].get_id())
                                    ...
                                    if ack_packet_id is None:
                                        print("ack_packet_id is None.")
...
final_file_name_output_1 = 'multi_hop_industrial_simulator/results/' + f'{year}_{month:02d}_{day:02d}_' + 'TL_output_dict' + '_final_UEs_' + str(final_number_of_ues) + '_Payload_' + str(payload_fq)+ '_contention_window_' + str(contention_window_int) + '_buffer_size_' + str(max_n_packets_to_be_forwarded) + '.txt'
final_file_name_output_2 = 'multi_hop_industrial_simulator/results/' + f'{year}_{month:02d}_{day:02d}_' + 'TL_averaged_data' + '_final_UEs_' + str(final_number_of_ues) +'_Payload_' + str(payload_fq)+ '_contention_window_' + str(contention_window_int) + '_buffer_size_' + str(max_n_packets_to_be_forwarded) + '.txt'
final_file_name_output_3 = 'multi_hop_industrial_simulator/results/' + f'{year}_{month:02d}_{day:02d}_' + 'TL_inputs' + '_final_UEs_' + str(final_number_of_ues) + '_Payload_' + str(payload_fq)+ '_contention_window_' + str(contention_window_int) + '_buffer_size_' + str(max_n_packets_to_be_forwarded) + '.txt'
